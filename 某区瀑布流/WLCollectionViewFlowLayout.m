...
- (void)prepareLayout {
    ...
    for (NSInteger i = 0; i < self.columnSection; i++) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat itemYy = [self.maxYDic[@0] floatValue] + self.rowSpacing;
    //设置attributes的frame
    if (indexPath.section == self.columnSection - 1) {
        attributes.frame = CGRectMake(itemX, itemY, itemWidth, itemHeight);
        //更新字典中的最大y值
        self.maxYDic[minIndex] = @(CGRectGetMaxY(attributes.frame));
        
        NSLog(@"1--%@------%@",minIndex,NSStringFromCGRect(attributes.frame));
        
    }else{
        attributes.frame = CGRectMake(0, itemYy, kScreenW, itemHeight);
        //更新字典中的最大y值
        self.maxYDic[@0] = @(CGRectGetMaxY(attributes.frame));
        self.maxYDic[@1] = @(CGRectGetMaxY(attributes.frame));
        
        NSLog(@"2--%@------%@",minIndex,NSStringFromCGRect(attributes.frame));
    }
